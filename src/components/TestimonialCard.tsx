...
const TestimonialCard: React.FC<TestimonialCardProps> = ({
  ...
  return (
    <div
      className="bg-white p-5 md:p-6 rounded-lg shadow-lg h-full flex flex-col transition-all duration-300 hover:shadow-xl transform hover:-translate-y-1 border border-transparent hover:border-gold/20"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className={`flex flex-col ${isRTL ? 'text-right' : 'text-left'} h-full`}>
        <div className={`mb-3 md:mb-4 text-gold transition-transform duration-300 ${isHovered ? 'scale-110' : ''}`}>
        ...
        <p className={`text-gray-600 mb-4 md:mb-6 text-sm md:text-base flex-grow transition-all duration-500 ${isHovered ? 'line-clamp-none' : 'line-clamp-4'}`}>
          {text}
        </p>
        <div className={`flex items-center ${isRTL ? 'justify-end' : ''} mt-4`}>
          <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden border-2 transition-all duration-300 ${isHovered ? 'border-gold shadow-gold/30 shadow-lg' : 'border-gold shadow-sm'}`}>
            {image ? (
              <LazyImage src={image} alt={name} className={`w-full h-full object-cover transition-transform duration-500 ${isHovered ? 'scale-110' : 'scale-100'}`} />
            ) : (
