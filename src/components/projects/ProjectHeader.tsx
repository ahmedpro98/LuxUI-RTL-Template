...
const ProjectHeader: React.FC<ProjectHeaderProps> = ({ project, isRTL }) => {
  return (
    <>
      ...
      <Link
        to="/projects"
        className={`inline-flex items-center text-neutral hover:text-primary transition-colors mb-8 ${isRTL ? 'flex-row-reverse' : ''}`}
      >
      ...
      <div className={`mb-12 ${isRTL ? 'text-right' : 'text-left'}`}>
        ...
        >
          <div className={`flex items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
            <Calendar className={`w-5 h-5 text-primary ${isRTL ? 'ml-2' : 'mr-2'}`} />
            <span>{project.year}</span>
          </div>
          <div className={`flex items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
            <Tag className={`w-5 h-5 text-primary ${isRTL ? 'ml-2' : 'mr-2'}`} />
            <span>{project.category}</span>
          </div>
          <div className={`flex items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
            <MapPin className={`w-5 h-5 text-primary ${isRTL ? 'ml-2' : 'mr-2'}`} />
